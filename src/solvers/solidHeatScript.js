...
import { FEAElementFunctions } from "../mesh/FEAResidualsScript.js";
import { FEAMesh } from "../mesh/FEAMeshScript.js";
 ...
 * @returns
 */
export function assembleSolidHeatMatrix2D(meshConfig, boundaryConditions) {
  ...
  // Extract node coordinates and nodal numbering (NOP) from the mesh data
  ...
  let nop = meshData2D.nodalNumbering;
  ...
  // Initialize the FEAElementFunctions class
  const elementFunctions = new FEAElementFunctions({
    dimension: "2D",
    order: 2,
  });

  // Calculate Gauss points and weights
  let gaussPointsAndWeights = elementFunctions.getGaussPointsAndWeights();
  ...
  for (let i = 0; i < ne; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      for (let k = 0; k < 3; k++) {
        ...
        let basisFunctionsAndDerivatives = elementFunctions.getBasisFunctions(
          gaussPoints[j],
          gaussPoints[k]
        );
    ...
    if (
      ...
      for (let l = 0; l < 3; l++) {
        ...
        let basisFunctionsAndDerivatives = elementFunctions.getBasisFunctions(gp1, gp2);
        basisFunction = basisFunctionsAndDerivatives.basisFunction;
        basisFunctionDerivKsi = basisFunctionsAndDerivatives.basisFunctionDerivKsi;
        basisFunctionDerivEta = basisFunctionsAndDerivatives.basisFunctionDerivEta;
